...
async function main() {
  ...
  const newVersion = inc(
    workspace.find('pinia-orm').data.version,
    bumpType || 'patch',
  )
  ...
  const branchExists =
    execSync(`git ls-remote --heads origin v${newVersion}`).toString().trim()
      .length > 0
  if (!branchExists) {
  ...
  const currentPR = (
    await $fetch(
      `https://api.github.com/repos/CodeDredd/pinia-orm/pulls?head=v${newVersion}`,
    )
  ).find((pull) => pull.title === `v${newVersion}`)
  ...
  if (!currentPR) {
    return await $fetch(
      'https://api.github.com/repos/CodeDredd/pinia-orm/pulls',
      {
        method: 'POST',
        headers: {
  ...
  const githubResponse = await $fetch(
    `https://api.github.com/repos/CodeDredd/pinia-orm/pulls/${currentPR.number}`,
    {
      method: 'PATCH',
      headers: {
        Authorization: `token ${process.env.GITHUB_TOKEN}`,
